...
function VeriffCallbackContent() {
  ...
  useEffect(() => {
    ...
    const pollForResults = async (): Promise<void> => {
      ...
      try {
        const response = await fetch('/api/veriff/check-status', {
          method: 'POST',
        ...
        if (data.verification_status === 'completed' && data.can_monetize) {
          if (pollIntervalRef.current) {
            clearInterval(pollIntervalRef.current);
            pollIntervalRef.current = null;
          }
      ...
      <div className="text-center space-y-4">
        ...
        {status === 'success' && (
          <>
            ...
            <p className="text-sm text-gray-500">Redirecting to creator settings...</p>
          </>
        )}
        {status === 'error' && (
          <>
