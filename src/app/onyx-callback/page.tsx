...
function OnyxCallbackContent() {
  ...
  useEffect(() => {
    ...
    const returnUrl = searchParams.get('returnUrl') || searchParams.get('return_url') || window.location.origin;
    ...
    (async () => {
      try {
        const res = await fetch('/api/onyx/check-payment-status', {
          method: 'POST',
...
export default function OnyxCallbackPage() {
  return (
    <Suspense
      fallback={
        <div className="flex items-center justify-center min-h-screen">
