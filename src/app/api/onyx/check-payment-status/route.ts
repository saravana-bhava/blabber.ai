...
import { checkOnyxPaymentStatus } from '@/lib/payments/onyx';
...
import type { OnyxPaymentMetadata, OnyxTransactionType } from '@/lib/payments/onyx';
...
export async function POST(request: Request) {
  try {
    ...
    if (!pending) {
      return NextResponse.json({ error: 'Pending payment not found or already completed' }, { status: 404 });
    }

    const statusResult = await checkOnyxPaymentStatus(transactionId);

    if (statusResult.status === 'SUCCESS') {
      const metadata = pending.metadata_json as OnyxPaymentMetadata;
      const transactionType = pending.transaction_type as OnyxTransactionType;
      ...
      if (res.status === 200) {
        await supabaseAdmin.from('pending_onyx_payments').delete().eq('transaction_id', transactionId);
      }
      return res;
    }
    ...
    console.error('Onyx check-payment-status error:', err);
  ...
  switch (transactionType) {
    ...
    case 'subscription': {
      ...
      if (subscriptionId) {
        ...
        if (subUpdateErr) {
          console.error('Onyx check-payment-status subscription update failed:', subUpdateErr);
      ...
      if (payInsertErr) {
        console.error('Onyx check-payment-status subscription_payments insert failed:', payInsertErr);
