...
function AffiliateVeriffCallbackContent() {
  ...
  useEffect(() => {
    ...
    const pollForResults = async (): Promise<void> => {
      ...
      try {
        const response = await fetch('/api/affiliate/check-status', {
          method: 'POST',
        ...
        if (data.verification_status === 'completed') {
          if (pollIntervalRef.current) { clearInterval(pollIntervalRef.current); pollIntervalRef.current = null; }
          setStatus('success');
          setMessage('Affiliate verification completed successfully!');
          toast.success('Affiliate verification completed!');
          ...
          setStatus('error');
          setMessage(isUnderage ? 'You must be at least 18 years old.' : 'Verification was not successful. Please try again.');
        ...
        console.error('Error polling affiliate verification status:', error);
      }
    };

    void pollForResults();
    if (!pollIntervalRef.current) {
      pollIntervalRef.current = setInterval(() => void pollForResults(), 5000);
    }
      ...
      <div className="text-center space-y-4">
        ...
        {status === 'success' && (
          <>
            ...
            <p className="text-sm text-gray-500">Redirecting to your affiliate dashboard...</p>
          </>
        )}
        {status === 'error' && (
          <>
...
export default function AffiliateVeriffCallbackPage() {
  return (
    <Suspense fallback={<LoadingFallback />}>
      <AffiliateVeriffCallbackContent />
    </Suspense>
  );
}
